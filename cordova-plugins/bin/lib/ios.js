...
module.exports = function (Q, fs, path, plist, xcode) {
	...
	function build(config) {
		...
		var plistXml = plist.build({ StringsTable: "Root", PreferenceSpecifiers: buildItems(config.settings) });
		...
		return fs.exists('platforms/ios')
			...
			.then (function () {
				return fs.exists ('platforms/ios/Settings.bundle/.com.pearson.assess.cordova.appsettings');
			...
			.then(function () {
				return fs.writeFile('platforms/ios/Settings.bundle/.com.pearson.assess.cordova.appsettings', "");
			})
			.then(function () {return fs.writeFile('platforms/ios/Settings.bundle/Root.plist', plistXml); })

			// Write localization resource file
			.then(function () {
				var i18ns = config.i18n, promises = [];
				Object.keys(i18ns).forEach( function (lang) {

					var langVal = i18ns[lang],
						langKeys = Object.keys(langVal),
						str = [];
					
					langKeys.forEach(function (k) {
						str.push('"' + k + '" = "' + langVal[k] + '";')
					});
	...
	function clean(config) {
		return fs.exists('platforms/ios')
			// Check if Settings.bundle is generated by plugin
			.then (function () { return fs.exists ('platforms/ios/Settings.bundle/.com.pearson.assess.cordova.appsettings'); })
			...
			.then(function () { return fs.unlink('platforms/ios/Settings.bundle/.com.pearson.assess.cordova.appsettings'); })
			.then(function () {
				var i18ns = config.i18n, promises = [];
				Object.keys(i18ns).forEach( function (lang) {
