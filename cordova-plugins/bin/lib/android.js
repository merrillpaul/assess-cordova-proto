...
module.exports = function (context) {
	...
	function buildSettings(config) {
		var configJson = config.settings,
			screenNode = new ET.Element('PreferenceScreen'),
		...
		stringsArrays.forEach(function (stringsArray) {
			...
			titlesXml.set("name", "appsettings_" + stringsArray.name);
			valuesXml.set("name", "appsettings_" + stringsArray.name + 'Values');
	...
	function build(config) {
		...
		return fs.exists('platforms/android')
			...
			.then(function () { return fs.writeFile('platforms/android/res/xml/appsettings.xml', preferencesDocument.write()); })

			// Write localization resource file
			.then(function () { return fs.mkdir('platforms/android/res/values'); })
			.then(function (prefs) { return fs.writeFile('platforms/android/res/values/appsettings.xml', preferencesStringDocument.write()); })
	...
	function afterPluginInstall () {
		return fs.exists('platforms/android')
			...
			.then(function () { return fs.readFile(path.resolve(__dirname, '../../src/android/AppSettingsActivity.template')); })
			.then(function (tmpl) {
			...
			.then(function (data) {
				var androidPackagePath = "com.pearson.assess.cordova".replace (/\./g, '/');
				var activityFileName= path.join ('platforms/android/src', androidPackagePath, 'AppSettingsActivity.java');
	...
	function clean(config) {
		...
		return fs.exists('platforms/android')
			...
			.then(function () { return fs.unlink('platforms/android/res/xml/appsettings.xml'); })

			// Remove localization resource file
			.then(function (prefs) { return fs.unlink('platforms/android/res/values/appsettings.xml'); })
